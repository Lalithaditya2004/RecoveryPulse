...
export default function Home() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Credentials synced successfully!' });
    } catch (err) {
    ...
    <main className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl p-8 border border-slate-100">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">RecoveryPulse</h1>
          <p className="mt-2 text-sm text-slate-500">One-Click Gateway Setup & WhatsApp Sync</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-semibold text-slate-700">Founder Account Email</label>
            <input
              type="email"
              required
          ...
          <div className="border-t border-slate-100 pt-4">
            <label className="block text-sm font-semibold text-slate-700">Stripe/Lemon Squeezy Webhook Secret</label>
            <input
              type="password"
              className="mt-1 block w-full p-3 rounded-lg border border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-slate-900"
              placeholder="whsec_..."
          ...
          <div className="border-t border-slate-100 pt-4 space-y-4">
            <div>
              <label className="block text-sm font-semibold text-slate-700">Meta WhatsApp Permanent Token</label>
              <input
                type="password"
                className="mt-1 block w-full p-3 rounded-lg border border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-slate-900"
                placeholder="EAAbw..."
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700">WhatsApp Phone Number ID</label>
              <input
                type="text"
                className="mt-1 block w-full p-3 rounded-lg border border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-slate-900"
          ...
          {status.message && (
            <div className={`p-4 rounded-lg text-sm font-medium ${status.type === 'success' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'}`}>
              {status.message}
            </div>
          )}
