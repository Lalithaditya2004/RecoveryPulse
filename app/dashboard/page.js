...
export default function Dashboard() {
  ...
  const handleDeleteBusiness = async (stripeAccountId) => {
    // 1. Confirm with the user to prevent accidental clicks
    const isConfirmed = window.confirm(
      "Are you sure you want to remove this business? Automated recovery messages will stop immediately.",
    );
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .eq("user_email", user.email); // Extra security check

      if (error) throw error;

      // 3. Update the UI state to remove the card instantly without reloading the page
      setConfigs((currentConfigs) =>
        currentConfigs.filter(
          (config) => config.stripe_account_id !== stripeAccountId,
        ),
      );
    } catch (err) {
          ...
          {configs.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-12 text-center">
              ...
              {!tierInfo.isActive ? (
                ...
                <Link
                  href="/dashboard/edit"
                  className="px-6 py-3 bg-[#635BFF] text-white font-semibold rounded-lg shadow-sm"
                >
                  Connect with Stripe
                </Link>
              )}
                ...
                {configs.map((config) => (
                  <div key={config.stripe_account_id} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col group transition-all hover:border-slate-300">
                    ...
                    <div className="pt-4 border-t border-slate-100 flex justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                        onClick={() => handleDeleteBusiness(config.stripe_account_id)}
                        className="text-xs font-semibold text-rose-600 hover:text-rose-700 px-3 py-1.5 bg-rose-50 hover:bg-rose-100 rounded-md transition-colors"
                      >
                        Remove Business
                      </button>
                    </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
            {events.length === 0 ? (
              <div className="p-8 text-center text-slate-500 text-sm">
                No recovery events recorded in the last 30 days.
              </div>
            ) : (
              <>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {events.map((evt) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-slate-600">
                            ${evt.amount_due?.toFixed(2)}
                          </td>
                          <td className="px-6 py-4">
                            <span
                              className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                                evt.status === "whatsapp_sent" ||
                                evt.status === "success"
